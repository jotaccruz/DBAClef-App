...
class InventoryFeeds():
    ...
    def __init__(self,window,mysqlserver,mysqlusername,mysqlpsw):
        ...
        self.InventoryTree.column("srv_ins_port", minwidth=0,width=80)
        ...
        self.InventoryTree.column("srv_user", minwidth=0,width=65)
        ...
        self.InventoryTree.column("srv_os", minwidth=0,width=65)
        self.InventoryTree.heading("srv_os", text="OS",)
        self.InventoryTree.column("srv_frecuency", minwidth=0,width=65)
        ...
        self.InventoryTree.column("srv_type", minwidth=0,width=65)
        self.InventoryTree.heading("srv_type", text="TYPE",)
        self.InventoryTree.column("srv_location", minwidth=0,width=65)
    ...
    def get_selected_row(self):
        try:
            ...
            self.srv_pwdInput.insert(END,InventoryFeeds.selected_row['srv_pwd'])
    ...
    def view_command(self):
        ...
        for self.row in self.view():
            ...
            self.row[4],self.row[5],self.row[6],self.row[7],self.row[8],self.row[9],self.row[10],self.row[11],self.row[12],\
    ...
    def search_command(self):
        ...
        OR srv_instance='"+self.srv_instanceInput.get()+"'\
        ORDER BY srv_name;"
        for self.row in dbservers(sqlexec,InventoryFeeds.mysqlserver,InventoryFeeds.mysqlusername,InventoryFeeds.mysqlpsw):
            self.InventoryTree.insert("", END, values=(self.row[0],row[1],row[2],row[3],\
    ...
    def add_command(self):
        ...
        srv_updated_dt)\
        VALUES("+"'"+InventoryFeed.srv_nameInput.get()+"','"+srv_instanceInput.get()+"','"+srv_ins_portInput.get()+"','"+\
        srv_ip1Input.get()+"','"+srv_ip2Input.get()+"','"+srv_userInput.get()+"','"+encrypt_password(srv_pwdInput.get())+"','"+\
        srv_directoryInput.get()+"','"+str(srv_osInput.current())+"','"+str(srv_frecuencyInput.current())+"','"+\
        ...
        #print (sqlexec)
        dbserversCreate(sqlexec,InventoryFeeds.mysqlserver,InventoryFeeds.mysqlusername,InventoryFeeds.mysqlpsw)
        view_command()

    def update_command(self,mysqlserver,mysqlusername,mysqlpsw):
        self.mysqlserver = mysqlserver
        self.mysqlusername = mysqlusername
        self.mysqlpsw = mysqlpsw

        sqlexec ="UPDATE lgm_servers \
        SET srv_name="+"'"+srv_nameInput.get()+"'\
        ,srv_instance='"+srv_instanceInput.get()+"'\
        ,srv_ins_port='"+srv_ins_portInput.get()+"'\
        ,srv_ip1='"+srv_ip1Input.get()+"'\
        ,srv_ip2='"+srv_ip2Input.get()+"'\
        ,srv_user='"+srv_userInput.get()+"'\
        ,srv_pwd='"+srv_pwdInput.get()+"'\
        ,srv_directory='"+srv_directoryInput.get()+"'\
        ,srv_os='"+str(srv_osInput.current())+"'\
        ,srv_frecuency='"+str(srv_frecuencyInput.current())+"'\
        ,srv_bk_location='"+srv_bk_locationInput.get()+"'\
        ,srv_type='"+str(srv_typeInput.current())+"'\
        ,srv_location='"+str(srv_locationInput.current())+"'\
        ,srv_domain='"+srv_domainInput.get()+"'\
        ,srv_active="+str(srv_activeInput.current())+"\
        ,srv_created_dt='"+srv_created_dtInput.get()+"'\
        ,srv_updated_dt='"+srv_updated_dtInput.get()+"'\
        WHERE\
        id="+"'"+srv_idInput.get()+"'"
        for i in InventoryTree.get_children():
            InventoryTree.delete(i)
